...
selected_protein_label = st.sidebar.selectbox(
    ...
    protein_labels,
    index=None,
    placeholder="Select a protein to begin",
)

if selected_protein_label is None:
    st.title("HOF–Protein Compatibility Recommender")
    st.write(
        "Select a target protein from the sidebar to generate HOF recommendations. "
        "The default ranking route is growth-mediated encapsulation."
    )
    st.info(
        "Recommendations are descriptor-based screening hypotheses. "
        "Literature labels are used for display and validation only, not as scoring inputs."
    )
    st.stop()
